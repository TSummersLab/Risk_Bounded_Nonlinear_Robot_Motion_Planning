...
class NMPC_Steering:
    ...
    def NMPC_Steer(self, start_w, next_w, SigmaE=None, rewireFlag=False):
        """
        ...
        goal_toler = 0.1  # Desired distance tolerance to be closer to goal                
        ...
        # Since things were successfull, append the final state as if it reached perfectly
        if rewireFlag: 
            if LA.norm(next_states[-1, 0:2] - final_state[0:2]) <= goal_toler:
                x_k_hist.append(final_state)
                S_k_hist.append(SigmaE) # Same covar as the last covar
            else:
                steerOutput = {"means": None, "covar": None, "cost" : 0, "status": False}
