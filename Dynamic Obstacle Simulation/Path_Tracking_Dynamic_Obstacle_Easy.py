...
# -*- coding: utf-8 -*-
"""
Created on Sat Apr 23 12:33:46 2022

@author: venkat
"""

"""
Path tracking simulation with linear model predictive control
Inspired from Atsushi Sakai (@Atsushi_twi)
"""
...
DT = 0.2  # [s] time tick
constantObsVelocity = 0.2

# Vehicle parameters
LENGTH = 4.5*3/4  # [m]
WIDTH = 2.0*3/4  # [m]
BACKTOWHEEL = 1.0*3/4  # [m]
WHEEL_LEN = 0.3*3/4  # [m]
WHEEL_WIDTH = 0.2*3/4  # [m]
TREAD = 0.7*3/4  # [m]
WB = 2.5*3/4  # [m]
...
def update_state(state, a, delta, vehicleFlag):
    ...
    # Steer accordingly if its the vehicle or the egovehicle
    if vehicleFlag:        
        # Update the states from the computed controls a, delta    
        ...
        # Obstacle car moves in a straight line with constant velocity
        state.x = state.x 
        state.y = state.y + constantObsVelocity * DT
        state.yaw = state.yaw
        state.v = state.v        
...
def predict_motion(x0, oa, od, xref):
    ...
    for (ai, di, i) in zip(oa, od, range(1, T + 1)):
        state = update_state(state, ai, di, vehicleFlag=(True))
        xbar[0, i] = state.x
        xbar[1, i] = state.y
        xbar[2, i] = state.v
        xbar[3, i] = state.yaw
...
def do_simulation(cx, cy, cyaw, ck, sp, dl, goal, initial_state, obstacle_state):
    """
    Simulation
    ...
    yaw = [state.yaw]
    v = [state.v]
    obsx = [obstacle_state.x]
    obsy = [obstacle_state.y]
    obsyaw = [obstacle_state.yaw]
    obsv = [obstacle_state.v]
    t = [0.0]
    d = [0.0]
    a = [0.0]
    ...
    while MAX_TIME >= time:
        ...
        # Update the ego vehicle and obstacle vehicle state using the computed controls
        state = update_state(state, ai, di, vehicleFlag=True)
        obstacle_state = update_state(obstacle_state, 0, 0, vehicleFlag=False)
        ...
        v.append(state.v)
        obsx.append(obstacle_state.x)
        obsy.append(obstacle_state.y)
        obsyaw.append(obstacle_state.yaw)
        obsv.append(obstacle_state.v)
        t.append(time)
        d.append(di)
        a.append(ai)
        ...
        if show_animation:  
            ...
            if ox is not None:
                plt.plot(ox, oy, "xb", label="Look-ahead MPC Trajectory")
            ...
            # Plot the ego vehicle and the obstacle vehicle
            plot_car(state.x, state.y, state.yaw, steer=di, vehicleFlag=True)
            plot_car(obstacle_state.x, obstacle_state.y, obstacle_state.yaw, steer=0, vehicleFlag=False)
...
def get_reference_course(dl):
    ...
    ax = path_points[0,:-50]
    ay = -1*path_points[1,:-50]
    cyaw = [np.pi/2]*len(ax)
    ck = 0*ax
    # cx, cy, cyaw, ck, s = cubic_spline_planner.calc_spline_course(ax, ay, ds=dl)

    return ax, ay, cyaw, ck
...
def main():
    ...
    cx, cy, cyaw, ck = get_reference_course(dl)
    ...
    # Do the simulation
    t, x, y, yaw, v, d, a, obsx, obsy = do_simulation(cx, cy, cyaw, ck, sp, dl, goal,
