...
def Plotter(start_w, end_w, obst, boundaryInfo):
    ...
    if config.DRFlag:
        if config.estimatorSelector:
            ...
            if config.velocitySelector == 0:
                treefilename = 'DR_RRT_Star_Tree010.pkl'
                nodelistfilename = 'pathNodesList010.pkl'
                refpathfilename = 'waypts_ref_data010.pkl'
                pointsfilename = 'freePoints010.pkl'
            if config.velocitySelector == 1:
                treefilename = 'DR_RRT_Star_Tree020.pkl'
                nodelistfilename = 'pathNodesList020.pkl'
                refpathfilename = 'waypts_ref_data020.pkl'
                pointsfilename = 'freePoints020.pkl'
        else:
            # DR-EKF
            treefilename = 'DR_EKF_RRT_Star_Tree.pkl'
            nodelistfilename = 'ekf_pathNodesList.pkl'
            refpathfilename = 'ekf_waypts_ref_data.pkl'
            pointsfilename = 'ekf_freePoints.pkl'
    else:
        # CC-UKF
        treefilename = 'CC_RRT_Star_Tree.pkl'
        nodelistfilename = 'cc_pathNodesList.pkl'
        refpathfilename = 'cc_waypts_ref_data.pkl'
        pointsfilename = 'cc_freePoints.pkl'
            
    
    # Unbox Pickle file to load the nodeList data     
    infile = open(treefilename,'rb')
    nodeList = pickle.load(infile)
    infile.close()
    
    # Unbox Pickle file to load the pathnodeList data 
    infile = open(nodelistfilename,'rb')
    pathNodesList = pickle.load(infile)
    infile.close()     
    
    # Unbox Pickle file to load the waypts_ref_data
    infile = open(refpathfilename,'rb')
    waypts_ref_data = pickle.load(infile)
    infile.close()        
    
    # Unbox Pickle file to load the queried points
    infile = open(pointsfilename,'rb')
    ...
    obst_rect = Rectangle((obst[0]-obst[3]/2, obst[1]-obst[4]/2), obst[3], obst[4], lw=2, fc='k', ec='k', alpha = 0.1, label='Obstacle Start')
    ax.add_artist(obst_rect)
    
    # Load the obstacle motion data set from pickle file
    if config.velocitySelector == 0:
        obsDatafilename = 'obstacleMotionData010.pkl'
    if config.velocitySelector == 1:
        obsDatafilename = 'obstacleMotionData020.pkl'
    infile = open(obsDatafilename,'rb')
    obstacleList = pickle.load(infile)
    infile.close()
        
    # Plot the dynamic obstacle as rectangle
    # Get the number of hops
    hopNumbers = 0
    for refNode in pathNodesList:
        hopNumbers = hopNumbers + refNode.means.shape[0]
    (lox,loy,lwd,lht) = obstacleList[hopNumbers-1]
    obst_rect = Rectangle((lox, loy), lwd, lht, lw=2, fc='k', ec='k', alpha = 0.5, label='Obstacle End')
    ax.add_artist(obst_rect)
...
if __name__ == '__main__':
    ...
    obs_w = [-75.0, -55.0, Wrap_Angle(180), 1.02, 2.49]
