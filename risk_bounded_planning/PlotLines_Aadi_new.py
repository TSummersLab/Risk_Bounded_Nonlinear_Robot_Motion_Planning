...
class BasicSynchronousClient(object):
    ...
    def render1(self, display, vehicle):
        """
...
#                                      line_thickness)            
    ...
    def render2(self, display, vehicle):
        """
        Transforms image from camera sensor and blits it to main pygame display.
        """
    ...
    def plotParametricEllipse(self, vehicle, center, orientation_angle = 0, color = carla.Color(r=0, g=0, b=0), nPoints = 20):
        
        
        # a is semi major axis and b is semi minor axis
        a = np.sqrt(self.y_ext**2 + self.x_ext**2) - 1
        b = self.x_ext + 1
        ...
        for i in range(len(data[0]) - 1):
            ...
            self.world.debug.draw_line(tailCoord, headCoord, life_time=1/10000, 
                                            thickness=0.2, color = color)
    # Plots ellipse with given center
    # Assumption: Ellipses are only in X-Y plane ( Z is assumed constant)
    def plotPath(self, vehicle, plot_coords):
        start_pt = plot_coords[0]
        for i in range(len(plot_coords) - 1):
            start_pt = plot_coords[i]
            end_pt = plot_coords[i + 1]
            # Coordinates at each step
            tailCoord = carla.Location(x = start_pt[0], y = start_pt[1], z = start_pt[2])
            headCoord = carla.Location(x = end_pt[0], y = end_pt[1], z = end_pt[2])
            self.world.debug.draw_line(tailCoord, headCoord, life_time=1/10000, 
                                            thickness = 0.1, color = carla.Color(r=0, g=0, b=0))
    def game_loop(self):
        """
        Main program loop.
        """
        ...
        try:
            ...
            start_loc = vehicles[0].get_location()
            plot_coords = np.array([[start_loc.x, start_loc.y, start_loc.z],
                                    [start_loc.x - 2, start_loc.y - 10, start_loc.z],
                                    [start_loc.x - 1, start_loc.y - 20, start_loc.z]])
            while True:
                self.world.tick()
                ...
                self.world.debug.draw_line(start_loc, new_location, life_time=10, 
                                            thickness=0.2, color=carla.Color(r=0, g=0, b=255))  
                
                # Plot ellipse
                yaw = np.radians(vehicles[0].get_transform().rotation.yaw - 90)
                center = np.array([vehicles[0].get_location().x, vehicles[0].get_location().y, vehicles[0].get_location().z])
                self.plotParametricEllipse(vehicles[0], center, yaw, carla.Color(r=0, g=255, b=100))
                
                # Plot ellipses
                for i in range(len(plot_coords)):
                    self.plotParametricEllipse(vehicles[0], plot_coords[i])
                    
                # Plot the path
                self.plotPath(vehicles[0], plot_coords)
                # Update the start location
                start_loc = new_location
                
                self.capture1 = True
                self.capture2 = True
                pygame_clock.tick_busy_loop(20)

                self.render1(self.display, vehicles[0])
                self.render2(self.display, vehicles[0])            
