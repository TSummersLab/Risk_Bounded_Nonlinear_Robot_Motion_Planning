...
def Plotter(start_w, end_w, obst, boundaryInfo):
    ...
    infile = open(filename,'rb')
    car_tracking_data = pickle.load(infile)
    infile.close() 
    ...
    # Plot start point
    ax.scatter(start_w[0], start_w[1], s=600, c='goldenrod', marker='^', alpha = 0.5, label='Start', zorder=20)
    ...
    for i in range(waypts_ref_data.shape[1]-1):
        ...
        plt.plot(xPtRef, yPtRef, color = 'blue', linewidth = 2.0)            
    
    ######################## Plot Tracked Trajectory ##########################                
    # Plot the complete node sequence
    for i in range(car_tracking_data.shape[1]-1):
        xPtRef = [car_tracking_data[0,i], car_tracking_data[0,i+1]]
        yPtRef = [car_tracking_data[1,i], car_tracking_data[1,i+1]]
        plt.plot(xPtRef, yPtRef, color = 'green', linewidth = 2.0)            
