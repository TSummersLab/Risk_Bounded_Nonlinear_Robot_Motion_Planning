# -*- coding: utf-8 -*-
"""
Created on Thu Apr 21 16:26:33 2022
...
SIGMAV = config.SIGMAV # TODO - do *10^5, *10^4, *1 - different noise levels
QLL = config.QLL
RLL = config.RLL
QTLL = config.QTLL
...
# MC_FOLDER = os.path.join('..', 'monte_carlo', 'env100005')  # NO DR + sigmas = 1e-5, lap
# MC_FOLDER = os.path.join('..', 'monte_carlo', 'env100000')  # NO DR + sigmas = 1e-7, lap
MC_FOLDER = os.path.join('..', 'monte_carlo', 'env100100')  # NO DR + sigmas = 1e-7, bern

# MC_FOLDER = os.path.join('..', 'monte_carlo', 'env000005')  # DR + sigmas = 1e-5, lap
...
def generate_disturbance_history(common_data, seed=None, dist=None, show_hist=False, sigma=None):
    if sigma is None:
        raise ValueError('covariance value required. none passed')
    rng = npr.default_rng(seed)
    sigma1 = sigma[0, 0]  # first entry in SigmaW
    x_ref_hist = common_data['x_ref_hist']
    T = x_ref_hist.shape[0]

    if dist is None:
        dist = "lap"  # "nrm", "lap", "gum"

    if dist == "nrm":
        w_hist = rng.multivariate_normal(mean=[0, 0, 0], cov=sigma, size=T)
    elif dist == "lap":
        l = 0
        b = (sigma1 / 2) ** 0.5
        ...
        choices = rng.random(size=[T, 3]) > 0.5
        w_hist = np.ones([T, 3])
        w_hist[:, 0] = 2*np.sqrt(sigma[0, 0])
        w_hist[:, 1] = 2*np.sqrt(sigma[1, 1])
        w_hist[:, 2] = 2*np.sqrt(sigma[2, 2])
        for i in range(T):
            if not choices[i, 0]:
                w_hist[i, 0] = 0
            if not choices[i, 1]:
            ...
            if not choices[i, 2]:
                w_hist[i, 2] = 0
        w_hist[:, 0] -= np.sqrt(sigma[0, 0])
        w_hist[:, 1] -= np.sqrt(sigma[1, 1])
        w_hist[:, 2] -= np.sqrt(sigma[2, 2])
    else:
...
def make_problem_data(T, num_trials, offset=0, dist='nrm'):
    ...
    for i in range(num_trials):
        idx = i + offset + 1
        w_hist = generate_disturbance_history(T, seed=2 * idx, dist=dist, sigma=SIGMAW)  # Process Noise
        v_hist = generate_disturbance_history(T, seed=2 * idx + 1, dist=dist, sigma=SIGMAV)  # Sensor Noise
...
if __name__ == "__main__":
    ...
    input_file = 'OptTraj_short_v1_0_1650551025_inputs'  # no DR ref traj
    # input_file = 'OptTraj_short_v1_0_1625163893_inputs'  # DR ref traj
    ...
    if run_flag:
        ...
        idx_list = make_problem_data(common_data, num_trials=num_trials, offset=trials_offset, dist='lap')
    ...
    for controller_str in controller_str_list:
        ...
        filename_out = 'path_plot_'+controller_str+'_lap_1'+'.png'
