...
MC_FOLDER = os.path.join('..', 'monte_carlo', 'env000274')
std_dev = 0.003
...
def trial(problem_data, common_data, controller, setup_time):
    ...
    if type(controller) in [OpenLoopController, LQRController]:
        ...
        time_start = time.time()
        nmpc_horizon = 10
        result_data = drrrtstar_with_nmpc(nmpc_horizon, x_ref_hist, u_ref_hist, n, m, T, w=w_hist, drnmpc = True, hnmpc = True)
        time_stop = time.time()
    elif controller.name == 'drnmpc':
        time_start = time.time()
        nmpc_horizon = 10
        result_data = drrrtstar_with_nmpc(nmpc_horizon, x_ref_hist, u_ref_hist, n, m, T, w=w_hist, drnmpc = True, hnmpc = False)
        time_stop = time.time()
...
if __name__ == "__main__":
    ...
    # controller_str_list = ['drnmpc', 'nmpc']
    # controller_str_list = ['nmpc', 'lqr']
    # controller_str_list = ['hnmpc', 'lqrm']
    # controller_str_list = ['open-loop', 'lqr', 'lqrm', 'nmpc', 'hnmpc']

    controller_str_list = ['open-loop', 'lqr', 'lqrm', 'nmpc']
    # controller_str_list = ['open-loop', 'lqr', 'lqrm', 'drnmpc', 'nmpc', 'hnmpc']
    ...
    for controller_str in controller_str_list:
        ...
        filename_out = 'path_plot_'+controller_str+'_'+std_dev_str+'.png'
